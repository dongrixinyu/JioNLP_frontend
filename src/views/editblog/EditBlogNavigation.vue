        ...
        >
          ...
          >
            ...
            <a-menu-item
              ...
              :key="i + '-' + j"
            >
              <router-link :to="'/editblog/' + chinese_name['blog_id']">{{
...
export default defineComponent({
  data() {
    return {
      $router: router,
      blog_index: {},
      openKeys: ref([0, 1, 2, 3, 4, 5, 6]),
      preOpenKeys: ref([0, 1, 2, 3, 4, 5, 6]),
      componentKey: 0,
    };
  },
  ...
  setup() {
    const state = reactive({
      ...
      selectedKeys: ["0-1"],
      openKeys: [0, 1, 2, 3, 4, 5, 6],
      preOpenKeys: [0, 1, 2, 3, 4, 5, 6],
    });
  ...
  created() {
    ...
    console.log("[EditBlogNavigation][created] start created");

    const attemptRequest = () => {
      if (attemptCount >= MAX_ATTEMPTS) {
        console.log("[EditBlogNavigation][created] Maximum attempts reached.");
      ...
      blog_backend({
        ...
        .then((response) => {
          if (response.data.is_ok) {
            ...
            console.log("[EditBlogNavigation][created] blog_index: ", this.blog_index);

            this.$nextTick(() => {
              // execute after the DOM is updated
              this.selectedKeys = [];
              this.openKeys = [0, 1, 2, 3, 4, 5, 6]
            });
            attemptCount += 5;
          } else {
            attemptCount++; // 增加尝试次数
            console.log(`[EditBlogNavigation][created] Retrying get_directory... Attempt ${attemptCount + 1}`);
            attemptRequest(); // 重试请求
          }
        })
        .catch(() => {
  ...
  updated() {
    ...
    const MAX_ATTEMPTS = 3; // 设置最大尝试次数
    console.log("[EditBlogNavigation][updated]", this.$route.params.first_directory);

    const attemptRequest = () => {
      if (attemptCount >= MAX_ATTEMPTS) {
        console.log("[EditBlogNavigation][updated] Maximum attempts reached.");
        return; // 如果已达到最大尝试次数，则不再继续重试
      }
      ...
      blog_backend({
        ...
        .then((response) => {
          if (response.data.is_ok) {
            this.blog_index = response.data.detail;
            console.log("[EditBlogNavigation][updated] blog_index: ", this.blog_index);
            this.$nextTick(() => {
              // execute after the DOM is updated
              console.log('[EditBlogNavigation][updated] blog_index: ', Object.keys(this.blog_index).length.toString());
              this.selectedKeys = [];
              this.openKeys = [0, 1, 2, 3, 4, 5, 6]
            });
            attemptCount += 5;
            ...
            attemptCount++; // 增加尝试次数
            console.log(`[EditBlogNavigation][updated] Retrying get_directory... Attempt ${attemptCount + 1}`);
            attemptRequest(); // 重试请求
          }
        })
        .catch(() => {
